...
import com.example.education.databinding.FragmentCoursesMentorsBinding
...
class Courses : Fragment() {
    ...
    private var binding: FragmentCoursesMentorsBinding? = null
    lateinit var coursesAdapter: CoursesAdapter
    lateinit var viewModel: MyViewModel
    lateinit var myDb: MyDb
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentCoursesMentorsBinding.inflate(inflater, container, false)
        ...
        binding?.apply {
            ...
            coursesAdapter = CoursesAdapter(coursesList, object : CoursesAdapter.OnItemPressed{
                override fun OnClick(course: Course) {
                    val fragment = Mentors()
                    val bundle = Bundle()
                    bundle.putString("name", course.name)
                    fragment.arguments = bundle
                    findNavController().navigate(R.id.action_courses_mentor_to_all_mentors_mentor, bundle)
                }
            })
